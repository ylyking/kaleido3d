...
namespace shaderlab
{
    ...
    { "RevSub", ngfx::BlendOperation::ReverseSubstract },
    ...
    { "DstColor",           ngfx::BlendFactor::DstColor },
    { "DstAlpha",           ngfx::BlendFactor::DstAlpha },
    { "OneMinusSrcColor",   ngfx::BlendFactor::OneMinusSrcColor },
    { "OneMinusSrcAlpha",   ngfx::BlendFactor::OneMinusSrcAlpha },
    { "OneMinusDstColor",   ngfx::BlendFactor::OneMinusDstColor },
    { "OneMinusDstAlpha",   ngfx::BlendFactor::OneMinusDstAlpha },
    };
    ...
    NodePtr Parser::ZWrite(std::unique_ptr<ParserNode> left, const Token & tok)
    {
        ...
        ngfx::DepthStencilState* depthStencil = ExtractDepthStencil(left.get());
        if (curWord == "On") {
            depthStencil->SetDepthWriteMask(ngfx::DepthWriteMask::All);
        } else {
            depthStencil->SetDepthWriteMask(ngfx::DepthWriteMask::Zero);
        }
        return left;
    }
    // ZClip NUMBER
    ...
    NodePtr Parser::Offset(std::unique_ptr<ParserNode> left, const Token & tok)
    {
        ...
        RS->SetDepthBiasSlope((float)depthSlope);
        RS->SetDepthBias((int32_t)depthBias);
    ...
    NodePtr Parser::Stencil(std::unique_ptr<ParserNode> left, const Token & tok)
    {
        ...
        if (LookAhead(Token::TOK_LEFT_BRACE)) {
            ...
            depthStencil->SetStencilEnable(false);
        } else {
    ...
    bool Parser::ParseBlendFactor(ngfx::RtBlendState& src)
    {
        ...
        if (tokStr == "Off") {
            ...
            if (!ParseBlendFactor(src.srcColorBlend, src.destColorBlend)) {
                return false;
            }
            if (LookAhead(Token::TOK_COMMA)) { // Alpha
                Consume();
                if (!ParseBlendFactor(src.srcAlphaBlend, src.destAlphaBlend)) {
                    return false;
                }
            }
            src.enable = true;
    ...
    NodePtr Parser::BlendOp(std::unique_ptr<ParserNode> left, const Token & tok)
    {
        ...
        auto CurTok = Cur();
        if (Match(Token::TOK_INTEGER)) {
            if (!ParseBlendOperation(BS->renderTargets[CurTok.Int()].colorBlendOp,
                BS->renderTargets[CurTok.Int()].alphaBlendOp)) {
                SpawnError("Failed to blend operation with Dest RenderTarget.");
                return left;
            }
        } else if (CurTok.GetType() == Token::TOK_IDENTIFIER) {
            if (!ParseBlendOperation(BS->renderTargets[0].colorBlendOp,
                BS->renderTargets[0].alphaBlendOp)) {
    ...
    static bool ParseColorMask(string_piece const& cmStr, ngfx::RtBlendState& rbs)
    {
        uint32_t mask = 0;
    ...
    NodePtr Parser::Cull(std::unique_ptr<ParserNode> left, const Token & tok)
    {
        ...
        ngfx::RasterizerState* RS = ExtractRaster(left.get());
        RS->SetCullMode(cullKeywords[curWord]);
        return left;
    }
}