...
#define LOG_TRACE() LogPrint(Log::Info, "Deconstruction", "----- %s %p ---\n", __FUNCTION__, this)
...
#define ERROR_STR(X) case VK_##X: return #X;
    switch (Result)
    {
        ...
        ERROR_STR(ERROR_INVALID_EXTERNAL_HANDLE_KHR)
    }
    return "Unknown";
#undef ERROR_STR
}
...
VkBlendFactor ConvertBlendFactorToVulkanEnum(BlendFactor const& e) {
    switch (e) {
...
public:
    VulkanLibrary1(VulkanDevice* pDevice, const void *pBlobData, size_t Size);
...
protected:
    void Init(const void *pBlobData, size_t Size);
...
public:
    ...
    void SetIndexBuffer(Buffer * pIndexBuffer, IndexBufferType type) override;
...
protected:
    ...
    VkResult CreatePipelineCache(void const* InData, size_t InSize)
    {
    ...
    VkResult RetrieveCache(void* OutData, size_t* OutSize)
    {
        if (Cache)
        {
...
{
public:
    VulkanBindTable(VulkanDevice* pDevice, VkDescriptorSet Ds, int numArgs);
...
public:
    ...
    Result CreateShaderCompiler(ShaderCompiler ** ppCompiler) override;
...
private:
    ...
    VkPhysicalDeviceGroupPropertiesKHR DeviceGroupProperties;
...
Result VulkanQueue::CreateCommandBuffer(CommandBuffer ** ppCmdBuffer)
{
	...
	*ppCmdBuffer = new VulkanCommandBuffer(this);
...
namespace ngfx {
	...
	{
		DeviceBuilder(VkPhysicalDevice pDevice) : Device(pDevice) 
		{
			...
			vkGetPhysicalDeviceProperties(Device, &Prop); // including limits
		...
		DeviceBuilder& EnableDebug(bool bEnable);
		DeviceBuilder& EnableMultiGpu(bool bMultiGpu);
		...
		Result Create(ngfx::Device ** ppDevice);
	...
	DeviceBuilder & DeviceBuilder::EnableDebug(bool bEnable)
	{
		return *this;
	}

	DeviceBuilder& DeviceBuilder::EnableMultiGpu(bool bMultiGpu)
	{
		return *this;
	}

	Result DeviceBuilder::Create(ngfx::Device ** ppDevice)
	{
    ...
    public:
		...
		DeviceBuilder CreateDeviceBuilder(VkPhysicalDevice physicalDevice);
        Result EnumDevice(uint32_t * count, Device ** ppDevice) override;
        Result CreateSwapchain(const SwapChainDesc * desc, CommandQueue * pCommandQueue, void * pWindow, SwapChain ** pSwapchain) override;
    ...
    , TableAllocator(nullptr)
{
    ...
    {
        const char* linkGpuExt = VK_KHR_DEVICE_GROUP_EXTENSION_NAME;
        ...
        deviceGroupCreateInfo.sType = VK_STRUCTURE_TYPE_DEVICE_GROUP_DEVICE_CREATE_INFO_KHR;
...
Result VulkanDevice::CreateShaderCompiler(ShaderCompiler ** ppCompiler)
{
    return Result();
}
...
DeviceBuilder ngfx::VulkanFactory::CreateDeviceBuilder(VkPhysicalDevice physicalDevice)
{
	return DeviceBuilder(physicalDevice);
}
...
void VulkanRenderPipeline::InitBlendState(const RenderPipelineDesc* pDesc)
{
    ...
    for (uint32_t i = 0; i < pDesc->numRenderTargets; i++)
    {
        ...
        BlendAttachState[i].srcAlphaBlendFactor = ConvertBlendFactorToVulkanEnum(Bs.renderTargets[i].srcAlphaBlend);
        BlendAttachState[i].dstAlphaBlendFactor = ConvertBlendFactorToVulkanEnum(Bs.renderTargets[i].destAlphaBlend);
        BlendAttachState[i].srcColorBlendFactor = ConvertBlendFactorToVulkanEnum(Bs.renderTargets[i].srcColorBlend);
        BlendAttachState[i].dstColorBlendFactor = ConvertBlendFactorToVulkanEnum(Bs.renderTargets[i].destColorBlend);
    }
...
VulkanBindTable::VulkanBindTable(VulkanDevice* pDevice, VkDescriptorSet Ds, int numArgs)
    : Handle(Ds)
    , Device(pDevice)
{
    Device->AddInternalRef();
	WriteDescriptorSets.resize(numArgs);
}
...
void VulkanBindTable::SetTexture(uint32_t index, ShaderType shaderVis, TextureView * textureView)
{
    ...
    VkDescriptorImageInfo Info = { 
		...
		static_cast<VulkanTextureView*>(textureView)->Handle, 
	...
	: OwningRoot(pQueue)
{
	...
	CHECK(vkAllocateCommandBuffers(OwningRoot->OwningRoot->Handle, &AllocInfo, &Handle));
}

VulkanCommandBuffer::~VulkanCommandBuffer()
{
	LOG_TRACE();
	vkFreeCommandBuffers(OwningRoot->OwningRoot->Handle, OwningRoot->Pool, 1, &Handle);
	OwningRoot->ReleaseInternal();
}
...
void VulkanRenderEncoder::SetIndexBuffer(Buffer * pIndexBuffer, IndexBufferType type)
{
    VulkanBuffer* pBuffer = static_cast<VulkanBuffer*>(pIndexBuffer);
    vkCmdBindIndexBuffer(OwningCommand->Handle, pBuffer->GetHandle(), 0, VkIndexType(type));
}
...
VulkanLibrary1::VulkanLibrary1(VulkanDevice * pDevice, const void * pBlobData, size_t Size)
    : Device(pDevice)
{
    Device->AddInternalRef();
    Init(pBlobData, Size);
...
void VulkanLibrary1::Init(const void * pBlobData, size_t Size)
{
...
Result BindTableAllocator::Allocate(const VulkanBindTableEncoder * pLayout, BindTable ** ppTable)
{
    ...
    if (DescriptorSet)
    {
        *ppTable = new VulkanBindTable(Device, DescriptorSet, pLayout->Info.bindingCount);
    }
