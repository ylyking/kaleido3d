...
from CI import *
from CI.Common import zipdir, copy_files_by_ext
...
#    src = os.getcwd()
    ...
    P = subprocess.Popen([find_cmake_exe(), 
...
def build_android_instance(abi, stl):
    src = os.getcwd()
    artifact_dir = os.path.join(src, 'artifacts', '_'.join([abi,stl]))
    build('release', abi, stl, 23, 23, src, artifact_dir)
