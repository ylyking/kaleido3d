...
void LogView::CreateUI()
{
    ...
    //m_Display->setMaximumBlockCount(500);
    m_Display->setReadOnly(true);
    m_Display->setLineWrapMode(QTextEdit::NoWrap);
    rootLayout->addWidget(m_Display);
   
    setLayout(rootLayout);
    m_Highlighter = new LogHighLighter(m_Display);
    connect(m_Input, SIGNAL(textEdited(const QString&)), SLOT(filterLogMessages(const QString&)));
}
