...
namespace k3d
{
    namespace mobi
    {
        namespace android
        {
            ...
            JdwpPacket JdwpPacket::NullPacket(ByteBuffer(0));
            void JdwpPacket::Write(SocketChannel * socket)
            {
                //socket->Send()
            }
            ...
            {
                I32 RecvLen = Read(buffer.m_Data, buffer.m_Capacity);
                if (RecvLen > 0)
                {
            ...
            void SocketChannel::SendJdwpPacket(JdwpPacket const & packet)
            {
                int Sent = Write(packet.Buffer, packet.Length);
                if (Sent != packet.Length)
                {
                    printf("Jdwp id:%d failed to send.", packet.Id);
                }
            }
            ...
            void AdbHelper::ListJDWPProcesses(IDevice* InDevice, std::function<void(DynArray<int>&pids)> callback)
            {
